...
def payment():
    ...
    cursor=mysql.connection.cursor()
    cursor.execute("insert into payment(bookingid,paymentStatus) values(%s,'paid')",(bookingid,))
    print("paid")
    mysql.connection.commit()
    cursor.close()
    return jsonify({"status": "success", "message": f"Payment recorded for booking ID {bookingid}"})
...
def createUser():
    ...
    prQint("fetched user entry . . . ")
    print(data)
    cursor = mysql.connection.cursor()
    dupName=cursor.execute("select * from users where username=%s",(userName,))
...
def fetch_room_price_from_chat(room_type):
    ...
    if price_data:
        return{
                "action":"chat_message",
                "message":f"price for {room_type} room is : rupees:{price_data[3]},\n"
